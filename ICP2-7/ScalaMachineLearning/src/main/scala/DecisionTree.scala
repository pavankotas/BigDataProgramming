...
object DecisionTree {
  ...
  def main(args: Array[String]) {
    ...
    val paramMap = new ParamGridBuilder()
      ...
      .addGrid(decisionTreeClassifier.maxDepth, Array(1,2,5, 10, 15))
      .addGrid(decisionTreeClassifier.minInstancesPerNode, Array(1, 2, 4,5,10))
      .build()
    //
    val cvModel = crossValidation(pipeline, paramMap, trainDf)
    println("train accuracy with cross validation" + MLUtilities.accuracyScore(cvModel.transform(trainDf), "label", "prediction"))
    println("test accuracy with cross validation " + MLUtilities.accuracyScore(cvModel.transform(crossDf), "Survived", "prediction"))
